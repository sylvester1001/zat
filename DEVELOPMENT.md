# ZAT 开发指南

## 当前进度

✅ **已完成**
- [x] 项目基础架构搭建
- [x] Tauri + Svelte 前端框架
- [x] Python FastAPI 后端框架
- [x] ADB 控制器（连接、截图、点击、滑动）
- [x] WebSocket 日志流
- [x] WebSocket 状态流
- [x] HTTP 截图端点（Debug 模式）
- [x] 前端 UI 组件（TaskControl, LogViewer, StatusBar, DebugPanel）

🚧 **待开发**
- [ ] 图像识别引擎（模板匹配）
- [ ] 任务配置系统（JSON 加载）
- [ ] 状态机实现
- [ ] 完整任务流程
- [ ] 错误处理和重试
- [ ] 配置文件管理

## 快速开始

### 1. 安装 Python 依赖

```bash
cd python-backend
python3 -m venv venv
source venv/bin/activate  # macOS/Linux
# 或 Windows: venv\Scripts\activate
pip install -r requirements.txt
```

### 2. 启动后端（终端 1）

```bash
cd python-backend
source venv/bin/activate
python main.py
```

后端将在 `http://127.0.0.1:8000` 启动

### 3. 启动前端（终端 2）

```bash
pnpm tauri dev
```

### 4. 连接模拟器

```bash
# 启动 MuMu 模拟器

# 连接 ADB
adb connect 127.0.0.1:16384

# 验证
adb devices
```

## 测试 API

### 使用 curl 测试

```bash
# 健康检查
curl http://127.0.0.1:8000/

# 连接设备
curl -X POST http://127.0.0.1:8000/connect

# 获取状态
curl http://127.0.0.1:8000/status

# 启动任务
curl -X POST "http://127.0.0.1:8000/start?task_name=farming"

# 停止任务
curl -X POST http://127.0.0.1:8000/stop

# 获取截图
curl http://127.0.0.1:8000/debug/screenshot -o screenshot.jpg
```

### 使用浏览器测试

打开 `http://127.0.0.1:8000/docs` 查看 FastAPI 自动生成的 API 文档

## 项目架构

### 通信流程

```
┌─────────────────────────────────────────────────────┐
│                   Svelte 前端                        │
│  ┌──────────────┐  ┌──────────────┐                │
│  │ TaskControl  │  │  LogViewer   │                │
│  └──────────────┘  └──────────────┘                │
│  ┌──────────────┐  ┌──────────────┐                │
│  │  StatusBar   │  │ DebugPanel   │                │
│  └──────────────┘  └──────────────┘                │
└─────────────────────────────────────────────────────┘
         │                    │
         │ HTTP               │ WebSocket
         ↓                    ↓
┌─────────────────────────────────────────────────────┐
│              Python Backend (FastAPI)                │
│                                                       │
│  HTTP 端点:                                          │
│  ├─ POST /connect                                    │
│  ├─ GET  /status                                     │
│  ├─ POST /start                                      │
│  ├─ POST /stop                                       │
│  └─ GET  /debug/screenshot                           │
│                                                       │
│  WebSocket 端点:                                     │
│  ├─ /ws/log    (日志流)                             │
│  └─ /ws/state  (状态流)                             │
│                                                       │
│  核心模块:                                           │
│  ├─ ADBController   (ADB 控制)                      │
│  ├─ TaskEngine      (任务引擎)                      │
│  └─ LogBroadcaster  (日志广播)                      │
└─────────────────────────────────────────────────────┘
         │
         │ ADB
         ↓
┌─────────────────────────────────────────────────────┐
│              MuMu 模拟器 (127.0.0.1:16384)          │
│                   杖剑传说                           │
└─────────────────────────────────────────────────────┘
```

### 截图传输方案

**HTTP 拉取式（已实现）**

```
前端点击"刷新截图"
    ↓
GET /debug/screenshot?gray=false
    ↓
Python: adb exec-out screencap -p
    ↓
OpenCV: JPEG 编码（质量 65）
    ↓
返回 JPEG bytes
    ↓
前端显示图片
```

**优势：**
- ✅ 按需获取，零性能压力
- ✅ 不使用 Base64，直接传输二进制
- ✅ 可以直接在浏览器访问测试

## 下一步开发

### Phase 1: 图像识别引擎

创建 `python-backend/core/vision.py`：

```python
class VisionEngine:
    def load_template(self, name: str, path: str)
    def match_template(self, screenshot, template_name, threshold=0.8)
    def find_all(self, screenshot, template_name, threshold=0.8)
```

### Phase 2: 任务配置系统

创建 `resources/tasks/farming.json`：

```json
{
  "name": "farming",
  "tasks": [
    {
      "task_name": "StartBattle",
      "recognition": {
        "method": "template_match",
        "template": "start_button.png",
        "threshold": 0.8
      },
      "action": {
        "type": "click",
        "target": "self"
      },
      "next": ["WaitBattleEnd"]
    }
  ]
}
```

### Phase 3: 状态机

更新 `python-backend/core/task_engine.py`：

```python
class StateMachine:
    IDLE -> START_GAME -> ENTER_STAGE -> IN_BATTLE -> RESULT -> IDLE
```

## 调试技巧

### 1. 查看 Python 日志

后端会在终端输出详细日志

### 2. 查看前端日志

打开浏览器开发者工具（F12）查看 Console

### 3. 测试 ADB 连接

```bash
# 查看设备
adb devices

# 手动截图测试
adb -s 127.0.0.1:16384 exec-out screencap -p > test.png

# 手动点击测试
adb -s 127.0.0.1:16384 shell input tap 640 360
```

### 4. 测试 WebSocket

使用浏览器 Console：

```javascript
// 测试日志流
const ws = new WebSocket('ws://127.0.0.1:8000/ws/log');
ws.onmessage = (e) => console.log(JSON.parse(e.data));

// 测试状态流
const ws2 = new WebSocket('ws://127.0.0.1:8000/ws/state');
ws2.onmessage = (e) => console.log(JSON.parse(e.data));
```

## 常见问题

### Q: ADB 连接失败？

```bash
# 1. 确认模拟器已启动
# 2. 确认 ADB 已安装
adb version

# 3. 重启 ADB 服务
adb kill-server
adb start-server

# 4. 手动连接
adb connect 127.0.0.1:16384
```

### Q: Python 依赖安装失败？

```bash
# 升级 pip
pip install --upgrade pip

# 单独安装 OpenCV
pip install opencv-python-headless

# 如果还是失败，尝试使用国内镜像
pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple
```

### Q: 前端无法连接后端？

1. 确认后端已启动（`http://127.0.0.1:8000`）
2. 检查 CORS 配置
3. 查看浏览器 Console 错误信息

## 代码规范

### Python

- 使用 Type Hints
- 使用 async/await
- 日志使用 logger，不要用 print
- 异常要有详细信息

### TypeScript

- 使用严格类型
- 组件要有清晰的 props 定义
- 使用 async/await 处理异步

## Git 提交规范

```
feat: 新功能
fix: 修复 bug
docs: 文档更新
refactor: 重构
test: 测试
chore: 构建/工具
```

## 性能优化建议

1. **截图优化**
   - 使用 `exec-out` 而不是 `pull`
   - JPEG 质量 60-70 足够
   - Debug 模式才传输截图

2. **识别优化**
   - 使用 ROI 减少搜索范围
   - 缓存模板图片
   - 考虑使用灰度图

3. **任务优化**
   - 合理设置延迟
   - 避免过度重试
   - 及时释放资源

## 资源

- [Tauri 文档](https://tauri.app/)
- [Svelte 文档](https://svelte.dev/)
- [FastAPI 文档](https://fastapi.tiangolo.com/)
- [OpenCV 文档](https://docs.opencv.org/)
- [ADB 文档](https://developer.android.com/tools/adb)
